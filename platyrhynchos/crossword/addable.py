...
class CrosswordAddable(Crossword):
    ...
    def __str__(self) -> str:
        ...
        for i in range(min_vertical, max_vertical + 1):
            ...
            for j in range(min_horizontal, max_horizontal + 1):
                ...
                if to_add == "+":
                    if (i, j) in self.clues_horizontal and (
                        i,
                        j,
                    ) not in self.clues_vertical:
                        to_add = "-"
                    elif (i, j) not in self.clues_horizontal and (
                        i,
                        j,
                    ) in self.clues_vertical:
                        to_add = "|"
                t.append(to_add)
            table.append("".join(t))
        return "\n".join(table)
    ...
    def combineWRotated(
        ...
        other: Crossword,
        T: float,
        key: Callable[[Crossword], float] = lambda x: -x.size,
    ) -> CrosswordAddable:
    ...
    def combine(
        ...
        if random() > T:
            found = sorted(
                self._check_crossings(other),
                key=lambda x: self._calc_size_change(x[0], x[1], other.max),
            )
        else:
