...
async def best_of(find_word_tasks: list[Coroutine[Any, Any, list[tuple[str, ColRow]]]], colrow_prios):
    ...
    for result in asyncio.as_completed(find_word_tasks):
        for word, colrow in await result:
            if word is None:
                continue
            ranking.add(await calc_usefulness(colrow_prios, word, colrow), (word, colrow))
    if ranking.non_empty():
        logger.debug("Found words: {}", ranking.get_best())
        word, colrow = random.choices(ranking.get_best(), weights=ranking.get_scores())[0]
        return word, colrow
    else:
...
async def find_for_removal(
    ...
    colrow_prios = {c: e for e, c in enumerate(list(cruciverbalist.choose_colrows(crossword))[::-1])}
    find_word_tasks = [
        cruciverbalist.find_removable_words(colrow) for colrow in cruciverbalist.choose_colrows(crossword)
    ]
    return await best_of(find_word_tasks, colrow_prios)
...
async def generate_crossword(width: int, height: int, word_amount: int) -> CrosswordImprovable:
    ...
    while not qualifies_for_goal(crossword, (turn := turn + 1)) or len(crossword.words) < word_amount:
        (addition_word, addition_colrow), (removal_word, _) = await asyncio.gather(
            find_for_addition(crossword, history), find_for_removal(crossword)
        )
